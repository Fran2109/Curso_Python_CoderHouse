...
class Post(models.Model):
    # Creo un campo de clave foránea que apunta al modelo Profile. Si se borra el perfil del usuario, se borrarán también los posts que el usuario haya creado.
    ...
    # Creo un campo que almacena la fecha y hora de creación del post. Se rellena automáticamente con la fecha y hora actual.
    fecha_creacion = models.DateTimeField(auto_now_add=True)
    # Creo un campo que almacena la fecha y hora de la última modificación del post. Se actualiza automáticamente cada vez que se modifica el post.
    fecha_modificacion = models.DateTimeField(auto_now=True)
    # Creo un campo que permite almacenar una imagen de fondo para el post. La imagen se guarda en la carpeta fondo dentro del directorio media y es opcional.
